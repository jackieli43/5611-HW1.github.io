...
TODO:
  ...
  3. Compute an avoidance force as follows:
       ...
       - Predict where both agent's will be at the moment of collision (time = TTC)
...
void setup(){
  ...
  agentPos[3] = new Vec2(500,500);
  agentPos[4] = new Vec2(400,400);
  agentPos[5] = new Vec2(600,700);
  goalPos[0] = new Vec2(200,420);
  goalPos[1] = new Vec2(120,120);
  goalPos[2] = new Vec2(220,220);
  goalPos[3] = new Vec2(600,200);
  goalPos[4] = new Vec2(500,400);
  goalPos[5] = new Vec2(500,100);
...
Vec2 computeAgentForces(int id){
  ...
  Vec2 goal_force = goalvel.minus(agentVel[id]).times(k_goal);
  Vec2 acc = goal_force;
  for (int i = 1; i < numAgents; i++) {
    acc = goal_force;
    if (id != i) {
      float time = computeTTC(agentPos[i],agentVel[i],agentRad,agentPos[id],agentVel[id],agentRad);
      if (time == -1) return acc;
      Vec2 A_future = agentPos[i].plus(agentVel[i].times(time));
      Vec2 B_future = agentPos[id].plus(agentVel[id].times(time));
      Vec2 relativeDir = (A_future.minus(B_future)).normalized();
      acc = acc.plus(relativeDir.times(k_avoid * (1/time)));
      return acc;
    }
    else {
      return acc;
...
void draw(){
  ...
  fill(20,200,150);
  for (int i = 0; i < numAgents; i++){
    circle(agentPos[i].x, agentPos[i].y, agentRadnew*2);
  }
}
