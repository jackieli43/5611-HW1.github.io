...
void draw(){
  ...
  fill(250,200,200); // WE ADD
  rect(boxTopLeft.x, boxTopLeft.y, boxW, boxH); // WE ADD
...
void keyPressed(){
  if (key == 'r'){
    placeRandomObstacles(); // ME ADD
    buildPRM(circlePos, circleRad, boxTopLeft, boxW, boxH); // ME ADD
    runBFS(closestNode(startPos),closestNode(goalPos)); // ME ADD
...
int closestNode(Vec2 point){
  ...
  int returner = 0;
  for (int i = 0; i < numNodes; i++) {
    if (nodePos[i] == point) continue;
      if (dist > point.distanceTo(nodePos[i])) {
...
boolean pointInBox(Vec2 boxTopLeft, float boxW, float boxH, Vec2 pointPos){
  Vec2 topRight = new Vec2(boxTopLeft.x + boxW,boxTopLeft.y);
  Vec2 bottomRight = new Vec2(boxTopLeft.x + boxW,boxTopLeft.y + boxH);

   if ((topRight.y < pointPos.y && bottomRight.y > pointPos.y) && (topRight.x > pointPos.x && boxTopLeft.x < pointPos.x)) return true;
...
void generateRandomNodes(Vec2[] circleCenters, float[] circleRadii, Vec2 boxTopLeft, float boxW, float boxH){
  for (int i = 0; i < numNodes; i++){
    ...
    boolean insideRect = pointInBox(boxTopLeft, boxW, boxH, randPos);
    while (insideAnyCircle || insideRect){
      randPos = new Vec2(random(width),random(height));
      insideAnyCircle = pointInCircleList(circleCenters,circleRadii,randPos);
      insideRect = pointInBox(boxTopLeft, boxW, boxH, randPos);
    }
    nodePos[i] = randPos;
  }
}
...
void connectNeighbors(){
  for (int i = 0; i < numNodes; i++){
    ...
    for (int j = 0; j < numNodes; j++){
      ...
      if (distBetween < 200) continue; // WE ADD
      hitInfo circleListCheck = rayCircleListIntesect(circlePos, circleRad, nodePos[i], dir, distBetween);
      hitInfo squareCheck = rayBoxIntersect(boxTopLeft, boxW, boxH, nodePos[i], dir, distBetween);
      if (!circleListCheck.hit && !squareCheck.hit){
        neighbors[i].add(j);
      }
    }
  }
